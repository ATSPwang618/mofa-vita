// Ported from wamsoft/shrinkCopy commit
// 007fbba3b3531b476d6b97e7ee80553868f31b2d. The upstream readme licenses
// this plug-in under the same terms as Kirikiri itself. Keep its published
// 1/256-pixel area-average behavior: retail scripts rely on the Layer API,
// not merely on Plugins.link succeeding.
...
static_assert(sizeof(unsigned long) == 4,
	"shrinkCopy's published BGRA pixel kernel requires 32-bit unsigned long");
...
struct ShrinkCopy : public LayerUtils
{
	...
	void* allocAvgBuffer(AvgInfoT* &hi, AvgInfoT* &vi, long w, long h) {
		hi = vi = 0;
		if (w <= 0 || h <= 0) return 0;
		const size_t horzCount = static_cast<size_t>(w);
		const size_t vertCount = static_cast<size_t>(h);
		if (horzCount > SIZE_MAX - vertCount) return 0;
		const size_t entryCount = horzCount + vertCount;
		if (entryCount > SIZE_MAX / sizeof(AvgInfoT)) return 0;
		void *ret = malloc(sizeof(AvgInfoT) * entryCount);
		if (!ret) return 0;
		hi = static_cast<AvgInfoT*>(ret);
		vi = hi + horzCount;
		return ret;
	}
...
struct LimitedShrink : public LayerUtils
{
	...
	bool resize() {
		const long width = siw / stepx + (siw % stepx != 0);
		const long height = sih / stepy + (sih % stepy != 0);
		tTJSVariant nw((tjs_int)width);
		tTJSVariant nh((tjs_int)height);
	...
	void copy() {
		...
		if (stepy <= 1) {
			...
			if (static_cast<size_t>(diw) > SIZE_MAX / 4u)
				throw std::bad_alloc();
			const size_t rowBytes = static_cast<size_t>(diw) * 4u;
			if (rowBytes > static_cast<size_t>(LONG_MAX))
				throw std::bad_alloc();
			const size_t rowCount = static_cast<size_t>(stepy < sih ? stepy : sih);
			if (rowBytes && rowCount > SIZE_MAX / rowBytes)
				throw std::bad_alloc();
			const size_t bufferBytes = rowBytes * rowCount;
			if (bufferBytes > static_cast<size_t>(LONG_MAX))
				throw std::bad_alloc();
			const long bpch = static_cast<long>(rowBytes);
			WrtRefT buf = new UnitT[bufferBytes];
			try {
				long div = sih/stepy;
	...
	void shrinkLineX(WrtRefT w, BufRefT r) {
		if (xdiv > 0) ShrinkLine(w, r, xdiv, stepx, stepx*4, 4);
...
namespace {
void mark_shrink_copy_ready()
{
	mofa_boot_trace("retail-shrinkcopy-ready");
}
}

NCB_POST_REGIST_CALLBACK(mark_shrink_copy_ready);
